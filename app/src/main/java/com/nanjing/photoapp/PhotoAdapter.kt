...
class PhotoAdapter(
    ...
    override fun onBindViewHolder(holder: VH, position: Int) {
        ...
        if (photo.isVideo() && photo.thumb_url == null) {
            ...
            Glide.with(holder.image.context).load(photo.gridThumbUrl()).fitCenter().into(holder.image)
        }
        ...
        holder.itemView.setOnLongClickListener {
            if (canManage && !selectionMode) {
                ...
                if (pos != RecyclerView.NO_POSITION) onLongPressEnterSelect(pos)
            }
            true
        }
    }
