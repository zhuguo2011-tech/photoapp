...
class AlbumDetailActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // 滑到接近底部自动加载下一批
        binding.recyclerPhotos.addOnScrollListener(object : androidx.recyclerview.widget.RecyclerView.OnScrollListener() {
            override fun onScrolled(rv: androidx.recyclerview.widget.RecyclerView, dx: Int, dy: Int) {
                ...
                if (!loadingPhotos && hasMorePhotos && firstVisible + visible >= total - 12) {
        ...
        lifecycleScope.launch {
            try {
                ...
                val response = ApiClient.service(this@AlbumDetailActivity)
                    .getPhotos(albumId, viewToken, offset, PAGE_SIZE)
                if (response.isSuccessful) {
                    val page = response.body() ?: emptyList()
                    if (reset) {
                        currentPhotos.clear()
                    }
                    currentPhotos.addAll(page)
                    hasMorePhotos = page.size >= PAGE_SIZE
                    adapter.updateData(ArrayList(currentPhotos))
